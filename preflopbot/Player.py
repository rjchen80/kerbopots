        ...
        while True:
            ...
            if word == "GETACTION":                    
                ...
                if self.ah._currentRound == 0:
                    effplayers = [x==None for x in self.ah.playerFoldActionId] and self.ah._activePlayers
                    nump = sum(effplayers)
                    ...
                    if self.ah._hasBRThisRound:
                        ...
                        while i<3 and i<=len(raiselist):
                            ...
                            if not raiselist[-i][-1] == 2:
                                ...
                                if sum(query_res) >= minweight:
                                    opraiselevel.append(1 - query_res[0]/query_res[1])
                                else:
                        ...
                        lowercallbound = max(pillar - 0.1, pillar - (1-pillar)/4.0)
                    ...
                    # if there is no initial raise
                        ...
                        print 'p_pfp', p_pfp
                        # decide to initial raise
                        if random.uniform(0,1) < p_pfp:
                            print 'Decided to initial raise'
                            for action in self.hand.legalActions:
                                a = action.split(':')
                                if a[0] == 'RAISE' or a[0] == 'BET':
                                    print a
                        ...
                        # decide not to initial raise
                        else:
                            print self.hand.legalActions
                            print done
                            print 'Decided NOT to initial raise'
                            if nump == 3:
                                k1 = self.pfp.irsp_3p[2]
                            else:
                                k1 = self.pfp.irsp_2p[2]
                            if equity_nump >= k1:
                                for action in self.hand.legalActions:
                                    a = action.split(':')
                                    if a[0] == 'CALL' and not done:
