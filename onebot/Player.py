...
#                for raiseEvent in self.hand.raiseHistory:
                ...
                discount = 1.0
                for x in equity_discounts:
                    discount = discount * x
                equity = (equity + (1-equity) * discount)**2 * equity
                print equity, discount
                ...
                for action in self.hand.legalActions:
                    ...
                    if a[0] == 'RAISE':
                        for dummy in self.hand.legalActions:
                            b = dummy.split(':')
                            if b[0] == 'CALL' and betvalue + int(b[1]) >= int(a[1]) and random.random() < equity:
                                done = True
                                betvalue = random.gauss(betvalue,float(int(a[2])-int(a[1]))/8)
                ...
                for action in self.hand.legalActions:
                    a = action.split(':')
                    if a[0] == 'BET' and not done and betvalue >= int(a[1]) and random.random() < equity:
                        done = True
                        betvalue = random.gauss(betvalue,float(int(a[2])-int(a[1]))/8)
