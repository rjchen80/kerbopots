    ...
    def run(self, s):
        ...
        while True:
            ...
            if word == "GETACTION":
                ...
                betvalue = self.hand.eval_bet(self.hand.eval_equity())
                for action in self.hand.legalActions:
                    a = action.split(':')
                    if a[0] == 'RAISE':
                        for dummy in self.hand.legalActions:
                            b = dummy.split(':')
                            if b[0] == 'CALL' and betvalue + int(b[1]) >= int(a[1]) and random.random() < self.hand.equity:
                                done = True
                                betvalue = random.gauss(betvalue,float(int(a[2])-int(a[1]))/4)
                ...
                for action in self.hand.legalActions:
                    a = action.split(':')
                    if a[0] == 'BET' and not done and betvalue >= int(a[1]) and random.random() < self.hand.equity:
                        done = True
                        betvalue = random.gauss(betvalue,float(int(a[2])-int(a[1]))/4)
                ...
                for action in self.hand.legalActions:
                    ...
                    if a[0] == 'CALL' and not done:
                        callvalue = self.hand.eval_call(int(a[1])-self.hand.bet)
                        if callvalue >= 0:
                            done = True
                            s.send(action+"\n")
