...
class Player:
    def run(self, s):
        ...
        while True:
            ...
            # With a certain percentage we bluff
...
#                for raiseEvent in self.hand.raiseHistory:
            ...
            (all_equity_discounts, foldIds, preflop_button_raise) = self.hand.eval_equity_discount()
            discount = 1.0
            for player in self.hand.playerNames:
                if not player == self.name and not self.hand.playerNameToId[player] in foldIds:
                    for round in all_equity_discounts.keys():
                        neglogx = - math.log(all_equity_discounts[round][self.hand.playerNameToId[player]])
                        neglogx = self.discount_adjust(neglogx, player, round)
                        x = math.exp(-neglogx)
                        discount = discount * x
            equity = max(0.0, equity + (1-equity) * discount + math.log(discount)/25)**2 * equity

            # 1- to reflect bluff
            reverseequity = min(1.0, 1.3 - equity)

            print equity, reverseequity, all_equity_discounts

            betvalue = self.hand.eval_bet(equity)
            bluff_betvalue = self.hand.eval_bet(reverseequity)
                ...
                if a[0] == 'RAISE':
                    for dummy in self.hand.legalActions:
                        ...
                        if b[0] == 'CALL' and betvalue + int(b[1]) >= int(a[1]):
                            ...
                            ## When good hand, only raise minimally
                            bluff_betvalue = random.gauss(bluff_betvalue, 2)
                            if actualequity > 0.95:
                                totalbet = max(int(a[1]), min(int((bluff_betvalue+betvalue)/1.5)+int(b[1]),int(a[2])))
                            else:
            ...
            for action in self.hand.legalActions:
                ...
                if a[0] == 'BET' and not done and betvalue >= int(a[1]):
                    ...
                    bluff_betvalue = random.gauss(bluff_betvalue, 2)
                    if actualequity > 0.95:
                        totalbet = max(int(a[1]), min(int((bluff_betvalue + betvalue)/1.5),int(a[2])))
                    else:
...
#                d = self.hand.eval_equity_discount()
#                all_equity_discounts = d[0], foldIds = d[1]
#                for player in self.hand.playerNames:
...
#                        self.discountAdjust[player] = self.discountAdjust[player] + (self.avgLogDiscount[self.name]-self.avgLogDiscount[player])*3.0/(self.hand.handId+50)
            self.update_avg_log_discount()
            self.update_discount_adjust()
            print self.preflop_button_raise

            if self.bluffed == True:
                self.bluffed = False
                    #if win, increase bluff_rate
                    #if loss, divide bluff_rate by 2.0
            ...
            for player in self.hand.playerNames:
                if not player == self.name and not self.hand.playerNameToId[player] in foldIds:
                    for round in all_equity_discounts.keys():
                        ...
                        discount = discount * x
            equity = max(0.0, equity + (1-equity) * discount + math.log(discount)/25)**2 * equity
            print equity, all_equity_discounts
            betvalue = self.hand.eval_bet(equity)
            for action in self.hand.legalActions:
                a = action.split(':')
                if a[0] == 'RAISE':
...
#                        self.discountAdjust[player] = self.discountAdjust[player] + (self.avgLogDiscount[self.name]-self.avgLogDiscount[player])*3.0/(self.hand.handId+50)
    ...
    def discount_adjust(self, neglogx, player, round):
        return neglogx - self.discountAdjust[player][round]
    def update_avg_log_discount(self):
#        d = self.hand.eval_equity_discount()
#        all_equity_discounts = d[0], foldIds = d[1]
        (all_equity_discounts, foldIds, preflop_button_raise) = self.hand.eval_equity_discount()
        for player in self.hand.playerNames:
            for round in ['preflop','flop','turn','river']:
                neglogx = self.discount_adjust(-math.log(all_equity_discounts[round][self.hand.playerNameToId[player]]), player, round)
                self.avgLogDiscount[player][round] = ((self.memoryLength-1)*self.avgLogDiscount[player][round] - neglogx) / self.memoryLength
        for player in self.hand.playerNames:
            if player == self.hand.playerNames[0]:
                self.preflop_button_raise[player] = (self.preflop_button_raise[player]*9 + preflop_button_raise)/10
        return
    def update_discount_adjust(self):
        for player in self.hand.playerNames:
            for round in ['preflop','flop','turn','river']:
                self.discountAdjust[player][round] = self.discountAdjust[player][round] + (self.avgLogDiscount[self.name][round] - self.avgLogDiscount[player][round]) / self.memoryLength
        return
    def parse_packet(self, data):
        ...
        if packet_values[0] == 'NEWGAME':
            ...
            self.avgLogDiscount = dict({self.players[0]:dict({'preflop':0.0, 'flop':0.0, 'turn':0.0, 'river':0.0}), self.players[1]:dict({'preflop':0.0, 'flop':0.0, 'turn':0.0, 'river':0.0}), self.players[2]:dict({'preflop':0.0, 'flop':0.0, 'turn':0.0, 'river':0.0})})
            self.discountAdjust = dict({self.players[0]:dict({'preflop':0.0, 'flop':0.0, 'turn':0.0, 'river':0.0}), self.players[1]:dict({'preflop':0.0, 'flop':0.0, 'turn':0.0, 'river':0.0}), self.players[2]:dict({'preflop':0.0, 'flop':0.0, 'turn':0.0, 'river':0.0})})
            self.memoryLength = 30.0
            self.preflop_button_raise = dict({self.players[0]:0.2, self.players[1]:0.2, self.players[2]:0.2})
            self.bluff_rate = 1.0
            self.bluffed = False
        elif packet_values[0] == 'NEWHAND':
            self.hand = Hand()
            self.hand.kerbopots = self
            self.hand.bet = 0
...
#                    self.hand.raiseHistory.append([stack, int(a[1])])
            ...
            winner = self.ah.winners[-1]
            # if self.bluffed == True:
            #     if self.hand.myname in winner:
            #         self.bluff_rate = min(0.9, self.bluff_rate + 0.05)
            #     else:
            #         self.bluff_rate = max(0.1, self.bluff_rate / 2.0)
            
            self.bluffed = False

            print "new bluff rate is " + str(self.bluff_rate)
