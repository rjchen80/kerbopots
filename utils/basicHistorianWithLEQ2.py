...
def writeResults(filename):
    ...
    wr = csv.writer(write, quoting = csv.QUOTE_ALL)
    headers = 'Name,Total Games,Win%,Fold% PreFlop,Fold% On Flop,Fold% On Turn,Fold% On River,Win% On Showdown,AVERAGE Stack Won,MAX pot won, Total #1s, Total #2s, Total #3s, Wins > 10, Wins < 10,bet ==2 percentage, bet 2 games, bet <= 4 percentage, bet <= 4 games, <=8\n'
    ...
    for keys, players in AllPlayers.items():
        ...
        stats = [players.name, players.totalGames, float(players.wins)/players.totalGames, players.foldBeforeFlop/float(players.totalGames),
            ...
            float(players.betupto8) / players.totalactions, players.betupto8]
        wr.writerow(stats)
    write.close()
...
writeResults('basichistoryDay8Total.csv')
